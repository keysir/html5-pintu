...
/************* 方法 ******************/
	...
	function imgSplit(img,cellW,cellH){		
		...
		for(var i=0;i<levelArr[levelNow][0];i++){
			for(var j=0;j<levelArr[levelNow][1];j++){				
				...
				$(cell).css({
					'width':cellW - 2, 'height':cellH - 2, 'left':j * cellW + 'px', 'top':i * cellH + 'px', "background":"url('"+img+"') no-repeat", 'backgroundPosition':(-j)*cellW + 'px ' + (-i)*cellH + 'px'});
				imgArea.append(cell);
			}
		}
		return arr;
	...
	function randomArr(len){
		...
		var arr = [];//记录随机数的数组变量
		var similar = 0;//记录随机数组中存在的未打乱比例，例如出现2,4,5,6,7...
		//产生随机数组
		for(var i=0;i<len;i++){
			order = Math.floor(Math.random()*len);
			if(arr.length > 0){
				while(jQuery.inArray(order,arr) > -1){
					order = Math.floor(Math.random()*len);
				}
			}
			arr.push(order);
		}
		//根据相似度，判断是否需要重新获取随机数组
		// for(var j=0;j<len;j++){
		// 	if(arr[j+1] - arr[j] == 1){
		// 		similar ++;
		// 	}else{
	...
	function cellExchange(obj,from,to){
		...
		//被交换图片移动
		obj.eq(to).css('z-index','30').animate({
...
/************* 变量 ******************/
	var levelArr = [,[5,4]];//记录游戏难度的数组,存储为[行数，列数]格式
...
/************* 初始化，此处为将图片拆为图片并显示难度 ******************/
	imgOrigArr = imgSplit('images/man.jpg',cellWidth,cellHeight);//拆分图片
...
/*************  事件  ******************/
	...
	btnStart.bind('mousedown',function(){
		...
		if(hasStart == '1'){
			...
			imgCells.bind('mousedown',function(e){		
                ...
                var x = touch.clientX;
                var y = touch.clientY;
				var cell_mouse_x = x- imgCells.eq(cellIndex_1).offset().left;
				var cell_mouse_y = y- imgCells.eq(cellIndex_1).offset().top;
	...
	btnLevel.bind('mousedown',function(){
		...
		imgOrigArr = imgSplit('images/man.jpg',cellWidth,cellHeight);
